...
const colorHexFromName = (name="")=>{
  ...
  const map = { black:"#000", white:"#fff", navy:"#001f3f", blue:"#0057ff", red:"#f00", green:"#080",
                gray:"#888", grey:"#888", beige:"#f5f5dc", brown:"#5c4033", yellow:"#ffd700",
                orange:"#ff7f00", pink:"#ffc0cb", purple:"#800080" };
  return map[k] || null;
};
...
function mergeByRoot(products){
  ...
  for (const p of products){
    ...
    if (!map.has(key)) { map.set(key, { ...p, name: key, colors: { ...p.colors } }); continue; }
...
/* ==== Tus mockups por color (opcional; si no hay, usa Printful) ==== */
const CUSTOM_IMAGES = {
  "Valtix V": {
...
router.get("/api/printful/products", async (req,res)=>{
  try{
    ...
    if (debug) {
      return res.json({
        debug: true,
        roots: normalized.map(p=>({ id:p.id, name:p.name, root:p.rootName, colors:Object.keys(p.colors||{}) }))
      });
    }
