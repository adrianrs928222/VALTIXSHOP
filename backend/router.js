...
function normalizeProduct(detail){
  ...
  for (const v of variants){
    ...
    // IMÁGENES por color (acumular)
    const files = Array.isArray(v?.files) ? v.files : [];
    const previews = files
      .map(f => f.preview_url || f.thumbnail_url || f.url)
      .filter(Boolean);

    // mini fallback por variante
    const firstImage = previews[0] || product.image || sp?.thumbnail_url || null;

    if (!colors[colorName]) colors[colorName] = { hex, image:firstImage, images:[], sizes:{} };
    if (firstImage && !colors[colorName].image) colors[colorName].image = firstImage;
    // acumular sin duplicados
    previews.forEach(u => { if(u && !colors[colorName].images.includes(u)) colors[colorName].images.push(u); });
  ...
  const cover =
    (firstColor && (colors[firstColor].images?.[0] || colors[firstColor]?.image)) ||
    sp?.thumbnail_url ||
  ...
  return {
    ...
    colors,       // { "Navy": {hex:"#001f3f", image:"...", images:[...], sizes:{S:123,...}} , ... }
    variant_map,
  };
}
